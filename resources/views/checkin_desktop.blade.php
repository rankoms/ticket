...
<head>
    ...
    <style>
        @font-face {
            ...
            src: url('{{ url('fonts/Roboto/Roboto-Light.ttf') }}');
            font-weight: 300;
        }

        @font-face {
            font-family: 'Roboto-Regular';
            src: url('{{ url('fonts/Roboto/Roboto-Regular.ttf') }}');
            font-weight: 400;
        }

        @font-face {
            font-family: 'Roboto-Medium';
            src: url('{{ url('fonts/Roboto/Roboto-Medium.ttf') }}');
            font-weight: 600;
        }

        @font-face {
            font-family: 'Roboto-Bold';
            src: url('{{ url('fonts/Roboto/Roboto-Bold.ttf') }}');
            font-weight: 700;
        }
...
<script>
    ...
    $('#form-voucher').on('submit', function(e) {
        e.preventDefault();
        var data = getJSON("{{ route('api.ticket.checkin') }}", {
            _token: '{{ csrf_token() }}',
            barcode_no: $('#voucher').val(),
            event: '{{ $event }}',
            category: '{{ $category }}'
        });
							...
							<div class="col-12">
								...
								<div class="form-floating mt-4">
									<input type="text" class="form-control text-center" id="category" placeholder="John Doe"
										aria-describedby="categoryHelp" value="${data.data.category}" readonly>
