...
<body class="antialiased">
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 sm:items-center sm:pt-0">
        <div class="container">
            <h1 class="text-center">Import Ticket</h1>
            <div class="row">
                <form action="{{ route('import.ticket') }}" id="form-import-ticket
                modal-import-ticket"
...
<script>
    $('#form-import-ticket').on('submit', function(e) {

        e.preventDefault();
        let formData = new FormData(this);

        $.ajax({
            url: "{{ route('import.ticket') }}",
            method: 'POST',
            data: formData,
            contentType: false,
            cache: false,
            ...
            success: function(response) {
                ...
                if (meta.status == 'error') {
                    $.each(meta.message, function(key, value) {
                        Swal.fire(
                            'Error',
                            value[0],
                            'error'
                    ...
                    $('#modal-import-ticket').modal('hide');
                    $('#file').val('');
                    table.ajax.reload();
                    Swal.fire(
                        meta.status == 'success' ? 'Success' : 'Warning',
                        meta.message,
                        meta.status == 'success' ? 'success' : 'warning'
                    ).then((result) => {
            ...
            error: function(error) {
                ...
                console.log(data);
                // $('#modal-request-attendance').modal('hide');
                if (data.errors) {
                    var idx = 0;
